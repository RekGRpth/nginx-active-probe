...
#include "ngx_http_upstream_conf_module.h"
#include "ngx_inet_slab.h"


static const ngx_str_t ngx_http_upstream_conf_params[] = {
...
static ngx_int_t
ngx_http_upstream_conf_is_shpool_range(ngx_http_request_t *r,ngx_slab_pool_t *shpool, void *p);
static ngx_int_t
ngx_http_upstream_conf_op_add(ngx_http_request_t *r, ngx_http_upstream_conf_op_t *op,
                            ngx_slab_pool_t *shpool, ngx_http_upstream_srv_conf_t *uscf);
static ngx_int_t
ngx_http_upstream_conf_op_remove(ngx_http_request_t *r, ngx_http_upstream_conf_op_t *op,
                               ngx_slab_pool_t *shpool, ngx_http_upstream_srv_conf_t *uscf);
static ngx_int_t
ngx_http_upstream_conf_op_update_param(ngx_http_request_t *r, ngx_http_upstream_conf_op_t *op,
                                     ngx_slab_pool_t *shpool, ngx_http_upstream_srv_conf_t *uscf);
...
ngx_int_t
ngx_http_upstream_conf_build_op(ngx_http_request_t *r, ngx_http_upstream_conf_op_t *op)
{
    ngx_uint_t                  i;
    size_t                      args_size;
    u_char                     *low;
    ...
    ngx_memzero(op, sizeof(ngx_http_upstream_conf_op_t));
    ...
    args = (ngx_str_t *)&ngx_http_upstream_conf_params;
    args_size = sizeof(ngx_http_upstream_conf_params) / sizeof(ngx_str_t);
...
ngx_int_t
ngx_http_upstream_conf_op(ngx_http_request_t *r, ngx_http_upstream_conf_op_t *op,
                        ngx_slab_pool_t *shpool, ngx_http_upstream_srv_conf_t *uscf)
{
    ngx_int_t  rc;
    ...
    case NGX_DYNAMIC_UPSTEAM_OP_ADD:
        rc = ngx_http_upstream_conf_op_add(r, op, shpool, uscf);
        break;
    case NGX_DYNAMIC_UPSTEAM_OP_REMOVE:
        rc = ngx_http_upstream_conf_op_remove(r, op, shpool, uscf);
        break;
#if 0
    ...
    case NGX_DYNAMIC_UPSTEAM_OP_PARAM:
        rc = ngx_http_upstream_conf_op_update_param(r, op, shpool, uscf);
        break;
                            ...
                            ngx_slab_pool_t *shpool, ngx_http_upstream_srv_conf_t *uscf)
{
    ...
    if (op->op_param & NGX_DYNAMIC_UPSTEAM_OP_PARAM_MAX_CONNS) {
        last->next->max_conns = op->max_conns;
    } else {
        last->next->max_conns = 1;
    }
                               ...
                               ngx_slab_pool_t *shpool, ngx_http_upstream_srv_conf_t *uscf)
{
    ...
    if (ngx_http_upstream_conf_is_shpool_range(r, shpool, target->name.data)) {
        ngx_slab_free_locked(shpool, target->name.data);
    }

    if (ngx_http_upstream_conf_is_shpool_range(r, shpool, target->sockaddr)) {
